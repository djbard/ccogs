...
////////////////////////////////////////////////////////////////////////
double distance(double a0, double d0, double a1, double d1,double conv_factor_angle) 
{

    double alpha = a0, delta0 = d0;
    double cos_d0 = cos(delta0), sin_d0 = sin(delta0), dist;

    double a_diff, sin_a_diff, cos_a_diff;
    double cos_d1, sin_d1, numer, denom, mult1, mult2;    
...
int main(int argc, char **argv)
{
    ...
    double hist_lower_range = 0.0000001;
    double hist_upper_range = 0;
    int nbins = DEFAULT_NBINS;
    double hist_bin_width = 0.05;
    int log_binning_flag = 0; // False

    double scale_factor = 1.0; // For if we need to convert input to arcsec or arcmin
    double conv_factor_angle = 57.2957795; // 180/pi // For if we need to convert arcdistance to arcsec or arcmin
    ...
    double temp_lo = hist_lower_range;
    ...
    double *h_alpha0, *h_delta0;
    double *h_alpha1, *h_delta1;
    ...
    */
    ...
    int size_of_galaxy_array = NUM_GALAXIES * sizeof(double);    
    printf("SIZE 0 # GALAXIES: %d\n",NUM_GALAXIES);

    h_alpha0 = (double*)malloc(size_of_galaxy_array);
    h_delta0 = (double*)malloc(size_of_galaxy_array);
    double temp0, temp1;
    ...
    h_alpha1 = (double*)malloc(size_of_galaxy_array);
    h_delta1 = (double*)malloc(size_of_galaxy_array);
    ...
    /*
       ...
       int size = NUM_PARTICLES * sizeof(double);    
       printf("SIZE0 # particles: %d\n",NUM_PARTICLES);

       h_alpha0 = (double*)malloc(size);
       h_delta0 = (double*)malloc(size);
    ...
    h_alpha1 = (double*)malloc(size);
    h_delta1 = (double*)malloc(size);
    ...
    unsigned long *hist;
    double hist_min = hist_lower_range;
    double hist_max = hist_upper_range;
    //int nbins;
    double bin_width = hist_bin_width;
